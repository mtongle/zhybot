...
from src.plugins.config import get_config, set_config
...
async def handle_ban(bot: Bot, event: GroupMessageEvent):
    ban_msg_list = get_config(self_id=bot.self_id, key='ban_msg_list')
    ...
    set_config(self_id=bot.self_id, ban_msg_list=ban_msg_list)
    await ban.finish(
...
async def handle_autoban(bot: Bot, event: GroupMessageEvent):
    group_ban_msg_list = get_config(self_id=bot.self_id, key='ban_msg_list').get(event.group_id) or []
